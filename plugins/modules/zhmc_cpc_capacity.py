...
options:
  ...
  state:
    description:
      ...
      - "* C(set): Ensures that the CPC has the specified specialty processor
           capacity and the specified software model, and returns the resulting
           processor capacity of the CPC."
      - "* C(facts): Does not change anything on the CPC and returns the
  ...
  software_model_direction:
    description:
      - "Indicates the direction of the capacity change for general purpose
         processors in C(software_model), relative to the current software
         model:"
      - "* C(increase): The specified software model defines more general
           purpose processors than the current software model."
      - "* C(decrease): The specified software model defines less general
           purpose processors than the current software model."
      - "Ignored when C(software_model) is null, not provided, or specifies
  ...
  specialty_processors:
    description:
      ...
      - "If the target number of processors is not installed in the CPC,
         the C(force) parameter controls what happens."
...
def split_target_processors(target_processors, current_processors):
    ...
    I all parameters and return values, dict key is the processor type and dict
    value is the number of processors.

    Parameters:
        target_processor_dict(dict): Dict with target numbers of the specialty
          processors.
...
def cpc_result_properties(cpc):
    ...
    for hmc_name in CPC_CAPACITY_PROPERTIES:
        value = cpc.get_property(hmc_name)
        result_props[hmc_name] = value
